...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    item8_id = Item.where(name: "Izod").first.id
    order_ids= OrderItem.where(item_id: item8_id).pluck(:order_id)
    user_ids = Order.find(order_ids).pluck(:user_id)
    users = User.find(user_ids).pluck(:name).sort
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    user_id = User.where(name: 'Mugatu').pluck(:id)
    items_for_user_3_third_order =  Order.where(user_id: user_id).third.items.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '20. returns the average amount for all orders for one user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    user3_id = User.where(name: "Mugatu").pluck(:id)
    average = Order.where(user_id: user3_id).average(:amount)
    # ------------------------------------------------------------
